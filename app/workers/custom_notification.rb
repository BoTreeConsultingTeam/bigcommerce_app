class CustomNotification
  ...
  def perform(order_id, store_hash, shipment_notification=false)
    store = Store.find_by(store_hash: store_hash)
    connection = prepare_connection(store, store_hash)
    if shipment_notification
      available_template = store.active_shipment_created_event_template.first
      template = available_template.present? ? available_template : Template.second
    else
      available_template = store.active_order_created_event_template.first
  ...
  def send_notification_email(email_to, email_from, email_subject, email_body, store)
    smtp_details = store.smtp_detail
